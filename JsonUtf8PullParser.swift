...
/*
fileprivate func test_JsonUtf8PullParser()
    {
    let json = "[ 1, 2, 3, { \"a\": 23, \"b\": \"a string\", \"c\": { \"d\": [ \"a0\", \"a1\", \"a2\", { \"e\": 17 } ], \"garbanzo factor\": 1.983329e-7 }, \"Test null\": null, \"Test true\": true, \"Test false\": false, \"Test single quotes\": '\"far out\" you say', \"Test double quotes\": \"'far out' you say\", \"Test empty string\": \"\" } ]"
    print(json);
    print();
    var i = 0
    var d1 = Data()
    ...
    repeat
        ...
        switch e
            {
            case JsonUtf8PullParser.Event.START_NVP:         print("\(e.name) \(p.getString().characters.count) \(p.getString())"); break
            case JsonUtf8PullParser.Event.NUMBER:            print("\(e.name) \(p.getDouble())");                                   break
            case JsonUtf8PullParser.Event.STRING:            print("\(e.name) \(p.getString().characters.count) \(p.getString())"); break
            case JsonUtf8PullParser.Event.SUPPLY_MORE_INPUT: print(e.name); p.supplyInput(utf8Json: d2, floor: 0, roof: d2.count);  break
            default:                                         print(e.name);                                                         break
            }
        }
    while e != JsonUtf8PullParser.Event.END_JSON
    }
test_JsonUtf8PullParser()
*/
